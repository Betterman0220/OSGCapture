...
// From osg screen capture example
...
public:
	...
	WindowCaptureCallback(const char* file_name, const char* extension, Mode mode, FramePosition position, GLenum readBuffer);
	...
	struct ContextData : public osg::Referenced
	{
		...
		void _Operation(const osg::Image& image, const unsigned int context_id)
		{
...
WindowCaptureCallback::WindowCaptureCallback(const char* file_name, const char* extension, Mode mode, FramePosition position, GLenum readBuffer)
	: _mode(mode),
	_position(position),
	_readBuffer(readBuffer),
	_filename(file_name),
